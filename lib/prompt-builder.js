...
export function createPromptBuilder(deps) {
    ...
    function collectRecentMessages(n, targetMessage = null) {
        ...
        const slice = chat.slice(Math.max(0, targetIndex - count + 1), targetIndex + 1);
        return slice
            .map(m => {
    ...
    function buildContextBlock(settings, targetMessage = null) {
        const chat = Array.isArray(getContext().chat) ? getContext().chat : [];
        const targetIndex = targetMessage == null ? -1 : chat.indexOf(targetMessage);
        const recent = collectRecentMessages(settings.contextMessages, targetMessage);
        ...
        let block = `${scope}:\n\n${recent || '(empty)'}`;
        if (targetIndex >= 0) {
            block = `[Selected scene anchor: message #${targetIndex + 1}]\n`
                + 'The final roleplay entry below is the exact moment to depict. '
                + 'Use earlier entries only for continuity; do not continue the scene beyond the selected message.\n\n'
                + block;
        }
    ...
    async function buildImagePrompt(settings, opts = {}) {
        const { signal, targetMessage = null } = opts;
        const template = String(settings.imagePromptTemplate || '').trim() || DEFAULT_IMAGE_PROMPT_TEMPLATE;
        const contextBlock = buildContextBlock(settings, targetMessage);
