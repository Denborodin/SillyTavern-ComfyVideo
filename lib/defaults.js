...
- Obey the supplied CAST CONSTRAINT exactly. An explicit list is the complete visible cast; exclude every unlisted person. "Show no people" forbids people, silhouettes, and background figures. In Auto mode, character cards are identity references only, not evidence that everyone is visible.
...
- Obey the supplied CAST CONSTRAINT exactly. When one character is explicitly selected, that character is the subject even if another character seems more prominent in the prose.
- Use the latest state for identity, current clothing, injuries, expression, props, and location.
- Show exactly one principal character. Exclude every other person; do not add background figures.
...
export const INTERACTION_IMAGE_PROMPT_TEMPLATE = `You write precise two- or three-character interaction prompts for natural-language text-to-image models such as Z-Image, Krea 2, and FLUX.2 Klein from roleplay chat.
...
- Obey the supplied CAST CONSTRAINT exactly. If two or three characters are explicitly selected, all of them are full participants in the interaction and every unlisted person is excluded.
- In Auto mode, prefer two important adult characters who are visibly present; include a third only when the latest state makes that person part of the same interaction. If only one character is present, describe that character interacting with the most important visible object instead of inventing another person.
...
Character 3 - [name or unique label], [different screen zone and depth]: the same categories when a third participant is selected or visibly involved.
...
- Obey the supplied CAST CONSTRAINT exactly. "Show no people" means no people, silhouettes, reflections, or background figures. With an explicit cast, include all and only the selected characters, while keeping the environment dominant. In Auto mode, cards are reference material only.
