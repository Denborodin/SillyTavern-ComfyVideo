...
export function createPromptBuilder(deps) {
                ...
                [
                    { role: 'system', content: template },
                    {
                        ...
                        content: contextBlock +
                            '\n\n[Pause the scene]\n' +
                            'Write the final image generation prompt for this exact moment using the guidelines. ' +
                            'Characters (appearance + current clothing) first, then poses/actions/composition/lighting, then background. ' +
    ...
    async function buildMotionPrompt(settings, opts = {}) {
        ...
        userBody += `\n\n[Pause the scene]\nClip length: ${seconds}s (about ${motionLines} motion lines / short sentences).`;
        if (sourceImagePrompt) {
            userBody += `\n\nSource still image prompt (identity, outfits, framing already locked — motion must fit this):\n${sourceImagePrompt}`;
        }
        userBody += '\n\nWrite the final motion prompt using the instruction format. '
            + 'Include a primary action arc, supporting motion, and one continuous camera plan. '
            + 'Natural language only — no JSON, no tags, no analysis. Output only the motion prompt.';
