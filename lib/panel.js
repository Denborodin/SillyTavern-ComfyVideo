...
export function createPanel(api) {
    ...
    function bindOnce() {
        ...
        overlay.querySelectorAll('.cv_image_action_btn[data-prompt-kind]').forEach(btn => {
            btn.addEventListener('click', async () => {
                ...
                const targetMessage = selectedTargetMessage;
                close();
                try {
    ...
    async function open(opts = {}) {
        await ensureDom();
        const { targetMessage = null } = opts;
        const chat = api.getContext().chat || [];
        selectedTargetMessage = chat.includes(targetMessage) ? targetMessage : null;
        ...
        selectedVideoSourceId = selectedTargetMessage && api.getMessageImageUrl(selectedTargetMessage)
            ? chat.indexOf(selectedTargetMessage)
            : null;
        paintResolution();
        paintImageQuality();
        paintLength();
