...
import { getMessageImageSource } from './media-source.js';
import {
    findLastRoleplayMessage,
    isComfyVideoMessage as detectComfyVideoMessage,
    messageSignature,
} from './scene-context.js';

export { getMessageImageSource } from './media-source.js';
 ...
 * @param {string|null} [opts.sourceMessageSignature] Frozen source-message signature
 * @param {boolean} [opts.sourceMustBeLatestRoleplay] Cancel if a newer roleplay message appeared
...
export async function attachGeneratedMedia(opts) {
    const {
        ...
        sourceChatId = null,
        sourceMessageSignature = null,
        sourceMustBeLatestRoleplay = false,
    } = opts;

    if (sourceChatId != null && typeof context.getCurrentChatId === 'function'
        && context.getCurrentChatId() !== sourceChatId) {
        throw new Error('The active chat changed before the generation completed.');
    }
    if (insertAfterMessage && sourceMessageSignature != null) {
        if (!context.chat?.includes(insertAfterMessage)) {
            throw new Error('The source message is no longer available in this chat.');
        }
        if (messageSignature(insertAfterMessage) !== sourceMessageSignature) {
            throw new Error('The source message changed before the generation completed.');
        }
        if (sourceMustBeLatestRoleplay && findLastRoleplayMessage(context.chat) !== insertAfterMessage) {
            throw new Error('The roleplay scene changed before the generation completed.');
        }
    }
    ...
    if (attachMode === 'last' && context.chat?.length) {
        ...
        // appendMediaToMessage is a SillyTavern jQuery API, not a DOM API.
        const messageElement = $(`.mes[mesid="${messageId}"]`);
        if (messageElement.length && appendMediaToMessage) {
...
export function getMessageImageUrl(message) {
    return getMessageImageSource(message)?.url || null;
}

export function isComfyVideoMessage(message) {
    return detectComfyVideoMessage(message);
}
