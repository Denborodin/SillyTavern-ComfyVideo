...
/**
 * Resolve a chat message by stable mesid first, then object identity / signature.
 * Prefer this over closed-over message references from older DOM injects.
 *
 * @param {any[]} chat
 * @param {{ id?: number|null, message?: any|null, signature?: string|null }} opts
 * @returns {{ message: any, id: number }|null}
 */
export function resolveChatMessage(chat, opts = {}) {
    if (!Array.isArray(chat) || !chat.length) return null;
    const id = Number(opts.id);
    const hasId = Number.isInteger(id) && id >= 0 && id < chat.length;
    const signature = opts.signature == null ? null : String(opts.signature);

    if (hasId) {
        const byId = chat[id];
        if (byId) {
            if (signature == null || messageSignature(byId) === signature) {
                return { message: byId, id };
            }
        }
    }

    if (opts.message && chat.includes(opts.message)) {
        return { message: opts.message, id: chat.indexOf(opts.message) };
    }

    if (signature) {
        for (let index = chat.length - 1; index >= 0; index--) {
            if (messageSignature(chat[index]) === signature) {
                return { message: chat[index], id: index };
            }
        }
    }
