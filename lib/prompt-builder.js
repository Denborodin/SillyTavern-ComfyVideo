...
export function createPromptBuilder(deps) {
    ...
    /**
     * @param {number} n
     * @param {any|null} targetMessage Message that anchors the generated scene.
     */
    function collectRecentMessages(n, targetMessage = null) {
        return collectRoleplayContext(getContext(), n, targetMessage).text;
    }

    function collectCharacterAppearance() {
        const ctx = getContext();
        const recent = collectRoleplayContext(ctx, 5);
        const participants = resolveCastParticipants(ctx, recent.messages);
        return buildCastContext(participants, { mode: 'auto', participantIds: [] }, true);
    }

    function getCastParticipants(settings, targetMessage = null) {
        const ctx = getContext();
        const recent = collectRoleplayContext(ctx, settings?.contextMessages, targetMessage);
        return resolveCastParticipants(ctx, recent.messages);
    }

    function buildContextBlock(settings, targetMessage = null, castSelection = null, options = {}) {
        const ctx = getContext();
        const recent = collectRoleplayContext(ctx, settings.contextMessages, targetMessage);
        const participants = resolveCastParticipants(ctx, recent.messages);
        const selection = normalizeCastSelection(castSelection);
        const includeCastReferences = options.includeCastReferences !== false && !!settings.includeCharacter;
        const castBlock = buildCastContext(participants, selection, includeCastReferences);
        const scope = targetMessage == null
            ? `Recent roleplay (last ${settings.contextMessages} messages)`
            : `Roleplay context ending at the selected message (up to ${settings.contextMessages} messages)`;
        let block = `${scope}:\n\n${recent.text || '(empty)'}`;
        if (recent.targetIndex >= 0 && targetMessage != null) {
            const roleplayAnchor = recent.lastRoleplayIndex >= 0
                ? `; latest included roleplay state is message #${recent.lastRoleplayIndex + 1}`
                : '';
            block = `[Selected source anchor: message #${recent.targetIndex + 1}${roleplayAnchor}]\n`
                + 'Use earlier entries only for continuity; do not continue the scene beyond the selected source.\n\n'
                + block;
        }
        return `${castBlock}\n\n${block}`;
    }
     ...
     * @param {any|null} [opts.targetMessage]
     */
    async function buildImagePrompt(settings, opts = {}) {
        const { signal, targetMessage = null, castSelection = null } = opts;
        const template = String(settings.imagePromptTemplate || '').trim() || DEFAULT_IMAGE_PROMPT_TEMPLATE;
        const contextBlock = buildContextBlock(settings, targetMessage, castSelection);
        ...
        const reply = await abortable(() => generateQuietPrompt({
            quietPrompt: userContent,
            responseLength: Number(settings.maxPromptTokens) || 700,
            skipWIAN: true,
        }), signal);
    ...
    async function buildMotionPrompt(settings, opts = {}) {
        const { sourceImagePrompt = '', signal, targetMessage = null } = opts;
        ...
        const contextBlock = buildContextBlock(settings, targetMessage, { mode: 'auto', participantIds: [] }, {
            includeCastReferences: !sourceImagePrompt,
        });
        const mode = settings.promptMode === 'quiet' ? 'quiet' : 'profile';
        ...
        userBody += '\nHold the opening beat on <Picture 1> so faces, body proportions, and clothing match the still, then begin the action. Do not morph identity to create motion.';
        ...
        try {
            ...
            const reply = await abortable(() => generateQuietPrompt({
                ...
                responseLength: Math.min(Math.max(Number(settings.maxPromptTokens) || 700, 120), 800),
                skipWIAN: true,
            }), signal);
