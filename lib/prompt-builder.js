...
const MOTION_SYSTEM =
    'You write short motion prompts for image-to-video. ' +
    'Output one line only: camera and subject motion. No quality tags, style essays, or image descriptions.';
...
export function createPromptBuilder(deps) {
    ...
    async function buildMotionPrompt(settings, signal) {
        if (settings.motionPromptMode !== 'auto') {
            return String(settings.fixedMotionPrompt || 'subtle natural movement, gentle camera motion').trim();
        }

        const template = String(settings.motionPromptTemplate || '').trim() || MOTION_SYSTEM;
        const contextBlock = buildContextBlock(settings);
        const mode = settings.promptMode === 'quiet' ? 'quiet' : 'profile';
        const fallback = String(settings.fixedMotionPrompt || 'subtle natural movement, gentle camera motion').trim();

        if (mode === 'profile') {
            if (!ConnectionManagerRequestService || !settings.llmProfileId) {
                return fallback;
            }
            const result = await ConnectionManagerRequestService.sendRequest(
                settings.llmProfileId,
                [
                    ...
                    {
                        role: 'user',
                        content: `${contextBlock}\n\nWrite the motion prompt now. Output only the motion line.`,
                    },
                ],
                Math.min(Number(settings.maxPromptTokens) || 400, 120),
                llmOptions(settings, signal),
            );
            const text = extractText(result);
        ...
        const reply = await generateQuietPrompt({ quietPrompt: userContent });
        if (reply?.trim()) return cleanPrompt(reply);
        return fallback;
    }
