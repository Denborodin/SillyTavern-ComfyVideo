...
export function ensureLibraries(st) {
    ...
    if (!imagePreset) {
        ...
        const legacy = L.imagePrompts.find(p => p.name === 'Z-Image – RP scene');
        if (legacy) {
            legacy.name = IMAGE_PROMPT_PRESET_NAME;
            legacy.template = DEFAULT_IMAGE_PROMPT_TEMPLATE;
            imagePreset = legacy;
        } else {
            imagePreset = {
                id: newId(),
        ...
        // Keep library preset text in sync with shipped default (user copies stay under other names)
        imagePreset.template = DEFAULT_IMAGE_PROMPT_TEMPLATE;
    }
    ...
    if (!motionPreset) {
        const legacyMotion = L.motionPrompts.find(p =>
            p.name === 'MiniMax H3 – I2V motion' || p.name === 'MiniMax H3 – I2V');
        if (legacyMotion) {
            legacyMotion.name = MOTION_PROMPT_PRESET_NAME;
            legacyMotion.template = DEFAULT_MOTION_PROMPT_TEMPLATE;
            motionPreset = legacyMotion;
        } else {
            motionPreset = {
                id: newId(),
