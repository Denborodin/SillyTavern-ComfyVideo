...
const LEGACY_IMAGE_TEMPLATE_FINGERPRINTS_BY_KIND = Object.freeze({
    scene: new Set(['4692:5eaf505a', '5569:7d19261d']),
    portrait: new Set(['1863:6b4e270d', '2222:0c713433']),
    interaction: new Set(['2241:a35e44c7', '2857:ac38196d']),
    environment: new Set(['2064:0a5615f9', '2408:1e01e4f1']),
});
...
export function ensureLibraries(st) {
    ...
    const previousMotionTemplate = String(motionPreset.template || '');
    if (LEGACY_MOTION_TEMPLATE_FINGERPRINTS.has(templateFingerprint(previousMotionTemplate))) {
        const activeUsesPreviousMotion = st.activeMotionPromptId === motionPreset.id
            && (!st.motionPromptTemplate?.trim() || st.motionPromptTemplate === previousMotionTemplate);
        motionPreset.template = DEFAULT_MOTION_PROMPT_TEMPLATE;
        if (activeUsesPreviousMotion) st.motionPromptTemplate = DEFAULT_MOTION_PROMPT_TEMPLATE;
    }
