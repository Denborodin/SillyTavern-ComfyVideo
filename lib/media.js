...
function preserveChatScrollPosition() {
    const chat = document.getElementById('chat');
    if (!chat) return () => {};
    const scrollTop = chat.scrollTop;
    return () => {
        const currentChat = document.getElementById('chat');
        if (currentChat) currentChat.scrollTop = scrollTop;
    };
}
 ...
 * @param {'last'|'new'|'after'} opts.attachMode
...
export async function attachGeneratedMedia(opts) {
    ...
    if (attachMode === 'after' && typeof context.reloadCurrentChat === 'function') {
        ...
        const restoreScroll = preserveChatScrollPosition();
        await context.saveChat();
        await context.reloadCurrentChat();
        restoreScroll();
        requestAnimationFrame(restoreScroll);
        // SillyTavern waits for media to settle before its final auto-scroll.
        // Restore once more after that window so older-message results stay in view.
        setTimeout(restoreScroll, 1250);
    } else {
